...
class TestValueBase64KeyCheck(unittest.TestCase):
    ...
    def test_value_check_n(self) -> None:
        line_data = LineData(config=None, path="dummy", file_type="", line="MIIXXXXX", info="",
                             line_num=1, line_pos=0, pattern=TestValueBase64KeyCheck.ALL_LINE_PATTERN )
        self.assertTrue(ValueBase64KeyCheck().run(line_data, DUMMY_ANALYSIS_TARGET))

    def test_value_check_p(self) -> None:
        line = (
            "'''MIICXAIBAAKBgQDFgQqUYexFziVtw\\n\\rnEz9XaYjmGdpGVWAqL1NX41LIxRAy1tbo1hCBUppqWGfn"
        ...
        line_data = LineData(config=None, path="dummy", file_type="", line=line, info="",
                             line_num=1, line_pos=0, pattern=TestValueBase64KeyCheck.ALL_LINE_PATTERN)

...
class TestMlValidator(unittest.TestCase):
    ...
    def test_ml_validator_simple_p(self):
        ...
        decision, probability = self.validate(candidate)
        self.assertAlmostEqual(0.9994745254516602, probability, delta=NEGLIGIBLE_ML_THRESHOLD)
        ...
        decision, probability = self.validate(candidate)
        self.assertAlmostEqual(0.9990401268005371, probability, delta=NEGLIGIBLE_ML_THRESHOLD)
    ...
    def test_ml_validator_auxiliary_p(self):
        ...
        self.assertAlmostEqual(0.9490059614181519, probability_batch[0], delta=NEGLIGIBLE_ML_THRESHOLD)
        ...
        sample_as_batch = [(candidate_key, [candidate, aux_candidate])]
        is_cred_batch, probability_batch = self.ml_validator.validate_groups(sample_as_batch, 2)
        self.assertAlmostEqual(0.9652721881866455, probability_batch[0], delta=NEGLIGIBLE_ML_THRESHOLD)
    ...
    def test_ml_validator_auxiliary_n(self):
        ...
        sample_as_batch = [(candidate_key, [candidate])]
        is_cred_batch, probability_batch = self.ml_validator.validate_groups(sample_as_batch, 2)
        self.assertAlmostEqual(0.9994876384735107, probability_batch[0], delta=NEGLIGIBLE_ML_THRESHOLD)

...
class TestApp(TestCase):
    ...
    def test_it_works_with_multiline_in_patch_p(self) -> None:
        ...
        expected = """
                    ...
                    rule: Token
                        | severity: high
                        | confidence: moderate
                        | ml_probability: 0.9995098114013672
                        | line_data_list:
                            [path: creds.py
                            | line_num: 5
                            | value: 'V84C7sDU001tFFodKU95USNy97TkqXymnvsFmYhQ'
    ...
    def test_depth_p(self) -> None:
        ...
        self.assertEqual("c16b86476bcd93566a4bf2072c2b3f81", binascii.hexlify(checksum).decode())
        normal_report = []
        sorted_report = []
        with tempfile.TemporaryDirectory() as tmp_dir:
            json_filename = os.path.join(tmp_dir, f"{__name__}.json")
    ...
    def test_rules_ml_p(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual("d309c0f6f0c60b17cb9f350907b4a483", checksum)

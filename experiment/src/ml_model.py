...
class MlModel(kt.HyperModel):
    ...
    def build(self, hp: Optional[Any]) -> Model:
        ...
        line_lstm = LSTM(units=self.line_shape[1], dtype=self.d_type, dropout=line_lstm_dropout_rate,
        ...
        variable_lstm = LSTM(units=self.variable_shape[1], dtype=self.d_type, dropout=variable_lstm_dropout_rate,
        ...
        value_lstm = LSTM(units=self.value_shape[1], dtype=self.d_type, dropout=value_lstm_dropout_rate,
        ...
        # first hidden layer
        dense_a = Dense(units=dense_units, activation=ReLU(), name="a_dense",
                        dtype=self.d_type)(joined_features)
        dropout_dense_a = Dropout(dense_a_dropout_rate, name="a_dropout")(dense_a)

        # second hidden layer
        dense_b = Dense(units=dense_units, activation=ReLU(), name="b_dense",
                        dtype=self.d_type)(dropout_dense_a)

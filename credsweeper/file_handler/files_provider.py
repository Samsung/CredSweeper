...
class FilesProvider(ABC):
    ...
    def __init__(self, paths: List[Union[str, Path, io.BytesIO]]) -> None:
        """Initialize Files Provider object for 'paths'.

        Args:
            paths: file paths list to scan OR io.BytesIO

        """
        self.paths = paths

    @property
    def paths(self) -> List[Union[str, Path, io.BytesIO]]:
        """paths getter"""
        return self.__paths

    @paths.setter
    def paths(self, paths: List[Union[str, Path, io.BytesIO]]) -> None:
        """paths setter"""
        self.__paths = paths

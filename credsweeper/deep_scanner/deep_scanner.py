...
class DeepScanner(ByteScanner, Bzip2Scanner, EncoderScanner, GzipScanner, HtmlScanner, LangScanner, PdfScanner,
    ...
    def scan(self, content_provider: ContentProvider, depth: int,
             recursive_limit_size: Optional[int]) -> List[Candidate]:
        ...
        """
        recursive_limit_size = recursive_limit_size if recursive_limit_size else RECURSIVE_SCAN_LIMITATION
    ...
    def structure_scan(
        ...
        for key, value in items:
            if isinstance(value, dict) or isinstance(value, list) or isinstance(value, tuple):
                ...
                candidates.extend(self.structure_scan(val_struct_provider, depth, recursive_limit_size))
